...
RUN git clone ${ZHENXUN_URL}.git ${ZHENXUN_DIR} \
    ...
    && wget ${SOURCE_URL}data_draw_card.tar.gz -O /root/data_draw_card.tar.gz \
    && wget ${SOURCE_URL}img_draw_card.tar.gz -O /root/img_draw_card.tar.gz \
    && pip install -r ${ZHENXUN_DIR}/requirements.txt \
    && tar -zxvf /root/data_draw_card.tar.gz -C ${ZHENXUN_DIR}/data/draw_card/ \
    && tar -zxvf /root/img_draw_card.tar.gz -C ${ZHENXUN_DIR}/resources/img/draw_card/ \
    && rm -f /root/*.tar.gz
